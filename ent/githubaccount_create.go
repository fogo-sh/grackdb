...
func (gac *GithubAccountCreate) AddRepositoryIDs(ids ...int) *GithubAccountCreate {
	gac.mutation.AddRepositoryIDs(ids...)
	return gac
}
...
func (gac *GithubAccountCreate) AddRepositories(r ...*Repository) *GithubAccountCreate {
	...
	return gac.AddRepositoryIDs(ids...)
}
...
func (gac *GithubAccountCreate) Save(ctx context.Context) (*GithubAccount, error) {
	...
	if len(gac.hooks) == 0 {
		...
		nv, ok := v.(*GithubAccount)
		if !ok {
			return nil, fmt.Errorf("unexpected node type %T returned from GithubAccountMutation", v)
		}
		node = nv
	}
	return node, err
...
func (gac *GithubAccountCreate) check() error {
	if _, ok := gac.mutation.Username(); !ok {
		return &ValidationError{Name: "username", err: errors.New(`ent: missing required field "GithubAccount.username"`)}
	}
	if v, ok := gac.mutation.Username(); ok {
		if err := githubaccount.UsernameValidator(v); err != nil {
			return &ValidationError{Name: "username", err: fmt.Errorf(`ent: validator failed for field "GithubAccount.username": %w`, err)}
		}
	}
	if _, ok := gac.mutation.OwnerID(); !ok {
		return &ValidationError{Name: "owner", err: errors.New(`ent: missing required edge "GithubAccount.owner"`)}
	}
	return nil
}
...
func (gac *GithubAccountCreate) createSpec() (*GithubAccount, *sqlgraph.CreateSpec) {
	...
	if nodes := gac.mutation.RepositoriesIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
