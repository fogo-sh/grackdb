...
func ID(id int) predicate.RepositoryTechnology {
	return predicate.RepositoryTechnology(func(s *sql.Selector) {
...
func HasTechnology() predicate.RepositoryTechnology {
	return predicate.RepositoryTechnology(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(TechnologyTable, FieldID),
			sqlgraph.Edge(sqlgraph.M2O, true, TechnologyTable, TechnologyColumn),
		)
		sqlgraph.HasNeighbors(s, step)
	})
}
