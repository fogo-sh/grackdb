...
func (gomc *GithubOrganizationMemberCreate) Save(ctx context.Context) (*GithubOrganizationMember, error) {
	...
	if len(gomc.hooks) == 0 {
		...
		v, err := mut.Mutate(ctx, gomc.mutation)
		if err != nil {
			return nil, err
		}
...
func (gomc *GithubOrganizationMemberCreate) check() error {
	if _, ok := gomc.mutation.Role(); !ok {
		return &ValidationError{Name: "role", err: errors.New(`ent: missing required field "GithubOrganizationMember.role"`)}
	}
	...
	if _, ok := gomc.mutation.OrganizationID(); !ok {
		return &ValidationError{Name: "organization", err: errors.New(`ent: missing required edge "GithubOrganizationMember.organization"`)}
	}
	if _, ok := gomc.mutation.AccountID(); !ok {
		return &ValidationError{Name: "account", err: errors.New(`ent: missing required edge "GithubOrganizationMember.account"`)}
	}
	return nil
}
