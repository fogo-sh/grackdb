...
func (rq *RepositoryQuery) sqlAll(ctx context.Context) ([]*Repository, error) {
	...
	if query := rq.withProject; query != nil {
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "project_repositories" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := rq.withGithubAccount; query != nil {
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "github_account_repositories" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.GithubAccount = n
			}
		}
	}
	...
	if query := rq.withGithubOrganization; query != nil {
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "github_organization_repositories" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.GithubOrganization = n
			}
		}
	}
	...
	if query := rq.withDiscordBots; query != nil {
		...
		for _, n := range neighbors {
			...
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "repository_discord_bots" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
	...
	if query := rq.withTechnologies; query != nil {
		...
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Technologies = []*RepositoryTechnology{}
		}
		query.withFKs = true
		...
		for _, n := range neighbors {
			...
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "repository_technologies" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "repository_technologies" returned %v for node %v`, *fk, n.ID)
			}
