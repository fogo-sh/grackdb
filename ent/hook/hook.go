...
// The DiscordAccountFunc type is an adapter to allow the use of ordinary
// function as DiscordAccount mutator.
type DiscordAccountFunc func(context.Context, *ent.DiscordAccountMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f DiscordAccountFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.DiscordAccountMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.DiscordAccountMutation", m)
	}
	return f(ctx, mv)
}
...
func (f ProjectContributorFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.ProjectContributorMutation)
	if !ok {
