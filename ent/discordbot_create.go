...
func (dbc *DiscordBotCreate) Save(ctx context.Context) (*DiscordBot, error) {
	...
	if len(dbc.hooks) == 0 {
		...
		v, err := mut.Mutate(ctx, dbc.mutation)
		if err != nil {
			return nil, err
		}
		nv, ok := v.(*DiscordBot)
...
func (dbc *DiscordBotCreate) check() error {
	if _, ok := dbc.mutation.AccountID(); !ok {
		return &ValidationError{Name: "account", err: errors.New(`ent: missing required edge "DiscordBot.account"`)}
	}
	if _, ok := dbc.mutation.ProjectID(); !ok {
		return &ValidationError{Name: "project", err: errors.New(`ent: missing required edge "DiscordBot.project"`)}
	}
	return nil
}
