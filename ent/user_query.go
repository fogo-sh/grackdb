...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withDiscordAccounts; query != nil {
		...
		for i := range nodes {
			...
			nodes[i].Edges.DiscordAccounts = []*DiscordAccount{}
		}
		query.withFKs = true
		...
		for _, n := range neighbors {
			...
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_discord_accounts" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_discord_accounts" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.DiscordAccounts = append(node.Edges.DiscordAccounts, n)
		}
	}
	...
	if query := uq.withGithubAccounts; query != nil {
		...
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.GithubAccounts = []*GithubAccount{}
		}
		query.withFKs = true
		...
		for _, n := range neighbors {
			...
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_github_accounts" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_github_accounts" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.GithubAccounts = append(node.Edges.GithubAccounts, n)
		}
	}
	...
	if query := uq.withProjectContributions; query != nil {
		...
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.ProjectContributions = []*ProjectContributor{}
		}
		query.withFKs = true
		...
		for _, n := range neighbors {
			...
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_project_contributions" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_project_contributions" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.ProjectContributions = append(node.Edges.ProjectContributions, n)
		}
	}
