...
func (pau *ProjectAssociationUpdate) check() error {
	...
	if _, ok := pau.mutation.ParentID(); pau.mutation.ParentCleared() && !ok {
		return errors.New(`ent: clearing a required unique edge "ProjectAssociation.parent"`)
	}
	if _, ok := pau.mutation.ChildID(); pau.mutation.ChildCleared() && !ok {
		return errors.New(`ent: clearing a required unique edge "ProjectAssociation.child"`)
	}
	return nil
}
...
func (pauo *ProjectAssociationUpdateOne) sqlSave(ctx context.Context) (_node *ProjectAssociation, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`ent: missing "ProjectAssociation.id" for update`)}
	}
