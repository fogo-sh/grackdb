...
func (dac *DiscordAccountCreate) Save(ctx context.Context) (*DiscordAccount, error) {
	...
	if len(dac.hooks) == 0 {
		...
		nv, ok := v.(*DiscordAccount)
		if !ok {
			return nil, fmt.Errorf("unexpected node type %T returned from DiscordAccountMutation", v)
		}
		node = nv
	}
	return node, err
...
func (dac *DiscordAccountCreate) check() error {
	if _, ok := dac.mutation.DiscordID(); !ok {
		return &ValidationError{Name: "discord_id", err: errors.New(`ent: missing required field "DiscordAccount.discord_id"`)}
	}
	if v, ok := dac.mutation.DiscordID(); ok {
		if err := discordaccount.DiscordIDValidator(v); err != nil {
			return &ValidationError{Name: "discord_id", err: fmt.Errorf(`ent: validator failed for field "DiscordAccount.discord_id": %w`, err)}
		}
	}
	if _, ok := dac.mutation.Username(); !ok {
		return &ValidationError{Name: "username", err: errors.New(`ent: missing required field "DiscordAccount.username"`)}
	}
	if _, ok := dac.mutation.Discriminator(); !ok {
		return &ValidationError{Name: "discriminator", err: errors.New(`ent: missing required field "DiscordAccount.discriminator"`)}
	}
	if v, ok := dac.mutation.Discriminator(); ok {
		if err := discordaccount.DiscriminatorValidator(v); err != nil {
			return &ValidationError{Name: "discriminator", err: fmt.Errorf(`ent: validator failed for field "DiscordAccount.discriminator": %w`, err)}
		}
	}
	return nil
}
