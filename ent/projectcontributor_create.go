...
func (pcc *ProjectContributorCreate) check() error {
	if _, ok := pcc.mutation.Role(); !ok {
		return &ValidationError{Name: "role", err: errors.New(`ent: missing required field "ProjectContributor.role"`)}
	}
	...
	if _, ok := pcc.mutation.ProjectID(); !ok {
		return &ValidationError{Name: "project", err: errors.New(`ent: missing required edge "ProjectContributor.project"`)}
	}
	if _, ok := pcc.mutation.UserID(); !ok {
		return &ValidationError{Name: "user", err: errors.New(`ent: missing required edge "ProjectContributor.user"`)}
	}
	return nil
}
